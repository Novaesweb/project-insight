alter table public.contratos
  add column if not exists requer_reassinatura boolean not null default false,
  add column if not exists reassinatura_motivo text;
...
begin
  ...
  needs_new_signature := contract_row.status <> 'assinado' or coalesce(contract_row.requer_reassinatura, false);

  if needs_new_signature then
    select nome, email
    ...
    set status = 'assinado',
        ...
        onboarding_started_at = case
          when had_existing_onboarding then onboarding_started_at
    ...
    values (
      ...
      'assinado',
      case
        when had_existing_onboarding then 'Contrato atualizado aprovado no portal'
        else 'Contrato aprovado no portal'
      end,
      case
        when had_existing_onboarding then coalesce(cliente_nome, signer_name) || ' assinou novamente a versão atualizada do contrato.'
        else coalesce(cliente_nome, signer_name) || ' aprovou o contrato no portal.'
      end,
      'cliente',
      ...
      jsonb_build_object(
        ...
        're_signed', had_existing_onboarding
      )
    );
