...
interface BriefingResponseViewProps {
  fieldDrafts: any[];
  answerMap: any;
  attachments: any[];
  formatDateTime: (date: string) => string;
  hasAnswerValue: (val: any) => boolean;
}

export function BriefingResponseView({
  fieldDrafts,
  answerMap,
  attachments,
  formatDateTime,
  hasAnswerValue,
}: BriefingResponseViewProps) {
  const sections = fieldDrafts.reduce<Array<{ name: string; fields: any[] }>>((acc, field) => {
    const existing = acc.find((s) => s.name === field.section_name);
    if (existing) {
      existing.fields.push(field);
    } else {
      acc.push({ name: field.section_name, fields: [field] });
    }
    return acc;
  }, []);
    ...
    <Card className="border-white/10 bg-white/[0.03]">
      <CardHeader>
        ...
        <CardDescription className="text-white/45">
          Perguntas respondidas e pendentes do briefing.
        </CardDescription>
      </CardHeader>
