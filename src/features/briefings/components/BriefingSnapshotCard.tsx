...
interface BriefingSnapshotCardProps {
...
export function BriefingSnapshotCard({
  snapshot,
  editor,
  attachments,
  selectedBriefing,
  formatDateTime,
}: BriefingSnapshotCardProps) {
  return (
    <Card className="border-white/10 bg-white/[0.03]">
      <CardHeader>
        <CardTitle className="text-white">Resumo recebido</CardTitle>
        <CardDescription className="text-white/45">
          Acompanhe respostas, anexos e datas do briefing selecionado.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4">
          ...
          <pre className="mt-3 whitespace-pre-wrap text-xs leading-relaxed text-white/70">
            {snapshot.briefing || editor.snapshot_briefing || "Nenhuma resposta consolidada ainda."}
          </pre>
        </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4">
          ...
          <pre className="mt-3 whitespace-pre-wrap text-xs leading-relaxed text-white/70">
            {snapshot.references || editor.snapshot_references || "Nenhum link consolidado ainda."}
          </pre>
        </div>
