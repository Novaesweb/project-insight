...
import { tickets, mensagensChat, type MensagemChat } from "@/lib/mock-data";
...
export default function ClienteSuporte() {
  ...
  const [localMsgs, setLocalMsgs] = useState<MensagemChat[]>(mensagensChat);
  const chatRef = useRef<HTMLDivElement>(null);

  const meusTickets = tickets.filter(t => t.clienteId === cliente.id);
  const ticketAtivo = meusTickets.find(t => t.id === selectedTicket);
  const ticketMsgs = localMsgs.filter(m => m.ticketId === selectedTicket);
  ...
  const enviarMensagem = () => {
    ...
    setLocalMsgs(prev => [...prev, nova]);
    setMensagem("");

    // Simula resposta do admin
    setTimeout(() => {
      setLocalMsgs(prev => [...prev, {
        ...
        texto: "Recebemos sua mensagem! Estamos analisando e retornamos em breve.",
        data: new Date().toISOString().replace("T", " ").slice(0, 16),
      }]);
    }, 2000);
  };
      ...
      <motion.div variants={fadeUp} initial="hidden" animate="show" className="space-y-4 h-full flex flex-col">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-white/40">{ticketAtivo.id}</p>
          </div>
          ...
          {ticketMsgs.map(m => (
            <div key={m.id} className={`flex ${m.remetente === "cliente" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${m.remetente === "cliente" ? "rounded-br-md" : "rounded-bl-md"}`}
                ...
                <p className="text-[9px] text-white/30 mt-1">{m.data}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-2">
          <Input
            ...
            className="flex-1 border-0 text-white placeholder:text-white/30"
        ...
        {meusTickets.map(t => (
          <Card key={t.id} className="border-[0.5px] border-white/[0.08] cursor-pointer hover:border-white/20 transition-all" style={{ background: "rgba(255,255,255,0.04)" }}
            ...
            <CardContent className="p-4">
              ...
              <p className="text-[10px] text-white/30 mt-2">{t.id} · {t.data}</p>
            </CardContent>
          </Card>
        ))}
        {meusTickets.length === 0 && <p className="text-sm text-white/40 text-center py-8">Nenhum ticket de suporte</p>}
      </div>
