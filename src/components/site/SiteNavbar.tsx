...
export default function SiteNavbar({ onOpenModal }: SiteNavbarProps) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "mx-auto flex h-14 w-full max-w-7xl items-center justify-between rounded-[1.4rem] border px-3 backdrop-blur-2xl transition-all duration-500 sm:h-16 sm:rounded-[1.7rem] sm:px-6",
          scrolled
      ...
      >
        ...
        >
          <OptimizedImage
            ...
            height={34}
            loading="eager"
            className="h-8 w-8 rounded-xl shadow-lg shadow-primary/20 sm:h-8 sm:w-8"
          />
          <div className="min-w-0 leading-none">
            <span className="site-gradient-text block truncate text-base font-black tracking-tight sm:text-lg">NovaesWeb</span>
            <span className="block truncate text-[8px] font-black uppercase tracking-[0.22em] text-white/40 sm:text-[9px] sm:tracking-[0.24em]">Studio digital</span>
          </div>
        </Link>
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            <div className="site-surface rounded-[1.55rem] border border-white/[0.08] p-4 sm:rounded-[1.8rem] sm:p-5 shadow-[0_24px_60px_rgba(0,0,0,0.5)]">
              ...
              <Button
                ...
                className="mt-5 h-12 w-full rounded-2xl border border-white/10 px-4 text-[11px] font-black uppercase tracking-[0.18em] text-white sm:text-xs sm:tracking-[0.22em]"
                style={{ background: "linear-gradient(135deg, rgba(220,38,38,0.92), rgba(107,33,168,0.9), rgba(236,72,153,0.88))" }}
              >
                Solicitar orcamento
              </Button>
