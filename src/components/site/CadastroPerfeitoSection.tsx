  ...
  return (
    <section id="cadastro" className="relative py-24 lg:py-32 overflow-hidden bg-gradient-to-b from-black via-gray-900 to-black">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        />
        <div className="absolute bottom-[10%] left-[5%] w-[500px] h-[500px] rounded-full opacity-[0.06]"
          style={{ background: 'radial-gradient(circle, #ff3366, transparent 70%)' }}
        />
        <div className="absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] rounded-full opacity-[0.04]"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          >
            ...
            {/* Progress bar */}
            <div className="h-1 w-full bg-purple-900/30">
              <motion.div
                animate={{ width: `${progressPercent}%` }}
                transition={{ duration: 0.4, ease: "easeOut" }}
                className="h-full rounded-r-full"
                style={{ background: 'linear-gradient(90deg, #a855f7, #ff3366, #ec4899)' }}
              />
            </div>
                  ...
                  return (
                    <div key={i} className="flex items-center gap-1">
                      <div className={cn(
                        ...
                        isCompleted && "text-purple-400/60",
                        ...
                        <div className={cn(
                          ...
                          isActive && "bg-gradient-to-r from-purple-500 to-red-500 text-white",
                      ...
                      {i < STEPS_SIDEBAR.length - 1 && (
                        <div className={cn("w-4 h-px transition-colors", isCompleted ? "bg-purple-500/30" : "bg-white/10")} />
                      )}
                    </div>
                  );
                })}
                  ...
                  <motion.div key="s0" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="text-center space-y-8">
                    <div className="relative mx-auto w-fit">
                      <div className="absolute -inset-4 rounded-3xl opacity-30"
                        style={{ background: 'linear-gradient(135deg, #a855f7, #ff3366, #ec4899)', filter: 'blur(12px)' }}
                      />
                ...
                {[1, 2, 3, 4].includes(currentStep) && (
                  <motion.div key={`s${currentStep}`} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.3 }} className="space-y-6 max-w-lg mx-auto w-full">
                    <div className="space-y-2">
                      <span className="text-[11px] font-bold text-purple-400 uppercase tracking-widest">Passo {currentStep} de 8</span>
                    ...
                    <div className="relative group">
                      {currentStep === 2 && <Mail className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-white/20 group-focus-within:text-purple-400/40 transition-colors" />}
                      <Input
                        autoFocus
                        className="h-16 bg-white/5 border-purple-500/20 rounded-xl text-xl font-semibold px-6 text-white focus:border-purple-400/40 focus:bg-white/10 transition-all placeholder:text-white/30"
