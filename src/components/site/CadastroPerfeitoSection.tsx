...
import {
  ...
  Users, Layout, Target, Package, Instagram, Search, HelpCircle, ArrowRight
} from "lucide-react";
...
export default function CadastroPerfeitoSection() {
  ...
  return (
    <section id="cadastro" className="min-h-screen bg-webnovax-bg flex items-center justify-center p-4 lg:p-8 font-inter text-white overflow-hidden relative">
            ...
            <AnimatePresence mode="wait">
              ...
              {currentStep === 0 && (
                <motion.div key="s0" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} className="text-center lg:text-left space-y-8">
                  ...
                  <div className="space-y-4">
                    <h1 className="text-4xl lg:text-6xl font-black tracking-tighter font-space leading-[0.85]">VAMOS <br /> <span className="opacity-20 italic">ELEVAR SEU</span> <br /> NEGÓCIO?</h1>
                    <p className="text-white/40 text-lg font-medium max-w-sm leading-relaxed">Sua jornada para a perfeição digital começa aqui.</p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 lg:gap-6">
                    {(currentStep === 5 ? NECESSIDADES : ORIGENS).map(opt => (
                      <button 
                        ...
                        className={cn("flex flex-col items-center justify-center p-8 lg:p-12 rounded-[2.5rem] border-2 transition-all gap-4 group relative overflow-hidden",
                          (currentStep === 5 ? form.necessidade : form.origem) === opt.id 
                            ? "bg-white/10 border-primary-webnovax shadow-2xl shadow-primary-webnovax/20" 
                            : "bg-white/[0.02] border-white/5 hover:border-white/20"
                        )}
                      >
                        <opt.icon className={cn("w-12 h-12 transition-transform group-hover:scale-110", 
              ...
              {currentStep === 6 && (
                <motion.div key="s6" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} className="space-y-12">
                   <div className="space-y-4">
                    <h2 className="text-sm font-black text-primary-webnovax uppercase tracking-[0.5em] opacity-80 pl-2 border-l-4 border-primary-webnovax/50">Passo 6 de 8</h2>
                  ...
                  <div className="grid grid-cols-1 gap-4 lg:gap-6">
                    {VOLUMES.map(opt => (
                      ...
                      >
                        ...
                        {form.volume === opt.id && (
                          <motion.div layoutId="vol-glow-site" className="absolute inset-0 bg-primary-webnovax/10 blur-2xl -z-10" />
                        )}
                      </button>
                    ))}
                  </div>
              ...
              {currentStep === 8 && (
                <motion.div key="s8" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -40 }} className="space-y-12 w-full max-w-4xl">
                   <div className="space-y-4">
                    <h2 className="text-sm font-black text-primary-webnovax uppercase tracking-[0.5em] opacity-80 pl-2 border-l-4 border-primary-webnovax/50">Passo Final</h2>
                    <h1 className="text-5xl lg:text-7xl font-black font-space tracking-tight leading-[0.9]">Fale um pouco mais sobre o projeto...</h1>
                  </div>
                  <div className="relative group">
                    <Textarea 
                      autoFocus
                      className="min-h-[300px] bg-white/[0.03] border-white/5 rounded-[2.5rem] p-12 text-2xl lg:text-3xl font-medium focus:border-primary-webnovax/50 transition-all placeholder:text-white/5 leading-relaxed shadow-inner"
                      placeholder="Quais seus objetivos, desafios ou sonhos para este projeto?"
                      value={form.mensagem}
                      onChange={(e) => updateForm("mensagem", e.target.value)}
                    />
                    <div className="absolute inset-0 rounded-[2.5rem] bg-primary-webnovax/5 blur-[50px] -z-10 opacity-0 group-focus-within:opacity-100 transition-opacity" />
                  </div>
                </motion.div>
              )}
                ...
                <motion.div key="s10" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center space-y-8 flex flex-col items-center">
                  ...
                  <div className="space-y-4">
                    ...
                    <p className="text-white/50 text-xl font-medium max-w-sm mx-auto leading-relaxed">
                      Um membro da nossa equipe já vai entrar em contato com você. Prepare-se para decolar! 🚀
                    </p>
                  </div>
                  <Button onClick={() => setCurrentStep(0)} className="h-20 px-16 rounded-[2rem] bg-white/5 border border-white/10 font-black uppercase tracking-widest text-sm hover:bg-white/10 transition-all">
                    Novo Cadastro
                  </Button>
                </motion.div>
              )}
