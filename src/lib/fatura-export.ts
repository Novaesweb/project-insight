...
export function exportFaturaPDF(fatura: FaturaData) {
  ...
  doc.text("novaesweb", w - 20, 18, { align: "right" });
  ...
  doc.setFont("helvetica", "normal");
  doc.text("Documento gerado automaticamente pelo sistema novaesweb", w / 2, 280, { align: "center" });
  ...
  const doc = new Document({
    sections: [{
      children: [
        ...
        new Paragraph({ spacing: { after: 100 }, children: [new TextRun({ text: "novaesweb — Soluções Digitais", size: 20, color: "888888" })] }),
        ...
        new Paragraph({ spacing: { before: 600 }, alignment: AlignmentType.CENTER, children: [new TextRun({ text: "Documento gerado automaticamente pelo sistema novaesweb", size: 16, color: "BBBBBB" })] }),
      ],
    }],
  });
